...
function detachedJson(value) {
    return JSON.parse(deterministicJson(value));
}
...
const NAMED_METHOD_FIELDS = Object.freeze({
    "prompts/get": "name",
    "resources/read": "uri",
    "tools/call": "name"
});
function visibleAscii(value, field, maximum = 128) {
    if (typeof value !== "string" || value.length < 1 || value.length > maximum || [
        ...value
    ].some((character)=>character.charCodeAt(0) < 0x20 || character.charCodeAt(0) > 0x7e)) {
        throw new ProtocolHelperError("INVALID_MCP_REQUEST", `${field} must be bounded printable ASCII`);
    }
    return value;
}
export function negotiateMcpVersion(offered) {
...
export function buildMcpRequest(input) {
    ...
    if (typeof input.requestId !== "string" && typeof input.requestId !== "number" || typeof input.requestId === "number" && !Number.isSafeInteger(input.requestId)) {
    ...
    if (input.version === MCP_CURRENT) {
        ...
        if (input.clientName !== undefined || input.clientVersion !== undefined || input.clientCapabilities !== undefined) {
...
const MCP_TASK_STATES = Object.freeze({
    working: [
        "ACTIVE",
        false,
        false,
...
function classifyTaskState(state, admitted, code) {
...
export function classifyMcpTaskState(state) {
    return classifyTaskState(state, MCP_TASK_STATES, "INVALID_MCP_TASK_STATE");
}
export function classifyA2aTaskState(state) {
    return classifyTaskState(state, A2A_TASK_STATES, "INVALID_A2A_TASK_STATE");
}
export function buildSseResumeHeaders(lastEventId) {
    if (typeof lastEventId !== "string" || lastEventId.length < 1 || lastEventId.length > 256 || [
        ...lastEventId
    ].some((character)=>character.charCodeAt(0) < 0x20 || character.charCodeAt(0) > 0x7e)) {
        throw new ProtocolHelperError("UNSAFE_RESUME_CURSOR", "Last-Event-ID must contain 1-256 printable ASCII characters");
    }
    return {
...
const EVENT_TYPES = Object.freeze({
    ...
    "harness.bundle-release.state.changed.v1": [
        "BundleRelease",
        false
    ],
    "harness.evidence.state.changed.v1": [
        "EvidenceRecord",
        false
    ],
    ...
    "harness.policy-bundle.state.changed.v1": [
        "PolicyBundle",
        false
    ],
...
function cloudFail(message) {
    throw new ProtocolHelperError("MALFORMED_CLOUD_EVENT", message);
}
function closedObject(value, fields, context) {
...
function requirePattern(value, pattern, context) {
...
function requireReferences(value, context) {
    if (!Array.isArray(value)) cloudFail(`${context} must be an array`);
    const seen = new Set();
    value.forEach((item, index)=>{
        const reference = closedObject(item, new Set([
            "kind",
            "id",
            "digest"
        ]), `${context}[${index}]`);
...
export function validateHarnessCloudEvent(value) {
    ...
    if (!Number.isSafeInteger(data.aggregateVersion) || data.aggregateVersion < 1) {
        cloudFail("data.aggregateVersion must be a positive safe integer");
    }
    const actor = closedObject(data.actor, new Set([
        "type",
        "id"
    ]), "data.actor");
    if (![
        ...
        "SYSTEM",
        "TENANT"
    ].includes(String(actor.type))) cloudFail("data.actor.type is invalid");
    ...
    if (data.transition !== null) {
        const transition = closedObject(data.transition, new Set([
            "from",
            "to"
        ]), "data.transition");
    ...
    const [aggregateRequirement, transitionMustBeNull] = EVENT_TYPES[event.type];
    if (typeof aggregateRequirement === "string" && data.aggregateKind !== aggregateRequirement || aggregateRequirement instanceof Set && !aggregateRequirement.has(String(data.aggregateKind))) cloudFail("event type and aggregate kind differ");
    ...
    return detachedJson(event);
}
export function serializeHarnessCloudEvent(value) {
